...
TF1* generateFitFunction(TEfficiency* teff_obj, Float_t gevmax){
...
void trouble_shoot_fix_fits(){
    TString efficiencyObjectFilePath = "../Root_Files/EfficiencyObjects.root";
    TString efficiencyFitsFilePath = "../Root_Files/EfficiencyFitsTryingtoFixthe2ActintBins.root";
    ...
    TF1* mu40thru50FitFunction = generateFitFunction( MET_Algmu40thru50Efficiency, 235.0);
    std::cout << "\nGenerating Fit for mu 50 to 60" << std::endl;
    TF1* mu50thru60FitFunction = generateFitFunction( MET_Algmu50thru60Efficiency, 295.0);
    std::cout << "Successfully generated fit functions" << std::endl;

    myfile->Close();
    TCanvas* canv5 = new TCanvas("canv5", "mu40thru50");
    canv5->DrawFrame(0,0,300.0,1.5);
    MET_Algmu40thru50Efficiency->Draw();
    gPad->Update();
    auto graph = MET_Algmu40thru50Efficiency->GetPaintedGraph();
    graph->SetMinimum(0);
    graph->SetMaximum(1);

    TCanvas* canv6 = new TCanvas("canv6", "mu50thru60");
    canv6->DrawFrame(0,0,300.0,1.5);
