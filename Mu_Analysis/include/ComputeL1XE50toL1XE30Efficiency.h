...
public :
   ComputeL1XE50toL1XE30Efficiency(){ 
       for (int i = 0 ; i < Number_Mu_Bins; i++){
           ...
           (TH1F*)HLT_ZB_L1XE30_MET_Distributions_by_Mubin[i] = new TH1F( Name , Title , met_dist_nbins , gevLow , gevHigh );
           (TH1F*)HLT_ZB_L1XE30_MET_Distributions_by_Mubin[i]->SetLineColor( Colors[i] );
           ...
           (TEfficiency*)L1XE50_Efficiency_Objects[i] = new TEfficiency( EfficiencyName , EfficiencyTitle , efficiency_nbins , gevLow , gevHigh );
           (TF1*)L1XE50_Efficiency_Fit_Objects[i] = new TF1();

           (TEfficiency*)L1XE50_Efficiency_Objects[i]->SetLineColor( Colors[i] );
           (TEfficiency*)L1XE50_Efficiency_Objects[i]->SetMarkerStyle( Colors[i] );
           (TF1*)L1XE50_Efficiency_Fit_Objects[i]->SetLineColor( Colors[i] );
       }
   }
   ~ComputeL1XE50toL1XE30Efficiency() { }
