...
Bool_t CorrectingDistributions::Process(Long64_t entry)
{
   ...
   if ( passedL1ZB()  && isGoodRun() ){
       if ( inMuRange( 0.0 , 10.0) ){
           MET_Correctedmu0thru10->Fill(*MET_Data, *HLT_noalg_zb_L1ZB_prescale);
       }
       if ( inMuRange( 10.0, 20.0) ){
           MET_Correctedmu10thru20->Fill(*MET_Data, *HLT_noalg_zb_L1ZB_prescale);
       }
       if ( inMuRange( 20.0 , 30.0) ){
           MET_Correctedmu20thru30->Fill(*MET_Data, *HLT_noalg_zb_L1ZB_prescale);
       }
       if ( inMuRange( 30.0, 40.0) ){
           MET_Correctedmu30thru40->Fill(*MET_Data, *HLT_noalg_zb_L1ZB_prescale);
       }
       if ( inMuRange( 40.0 , 50.0 ) ){
           MET_Correctedmu40thru50->Fill(*MET_Data, *HLT_noalg_zb_L1ZB_prescale);
       }
       if ( inMuRange( 50.0 , 60.0 ) ){
           MET_Correctedmu50thru60->Fill(*MET_Data, *HLT_noalg_zb_L1ZB_prescale);
       }
       if ( inMuRange( 60.0 , 70.0 ) ){
           MET_Correctedmu60thru70->Fill(*MET_Data, *HLT_noalg_zb_L1ZB_prescale);
       }
   }
