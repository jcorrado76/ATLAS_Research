       ...
       for ( int i = 0; i < Number_Mu_Bins ; i++ ){
           ...
           if ( inMuRange( muLow , muHigh ) ){
               for ( int j = 0 ; j < Number_Mu_Bins ; j++ ){
                   ((TH1F*)HLT_ZB_L1XE30_MET_Distributions_by_Mubin[j])->Fill(*cell_met , *HLT_noalg_L1XE30_prescale);
                   ((TEfficiency*)L1XE50_Efficiency_Objects[j])->Fill(*L1_met > L1XE , *cell_met);
               }
           }
       }
   }
...
{
    for (int i = 0 ; i < Number_Mu_Bins ; i++ ){
       (TF1*)L1XE50_Efficiency_Fit_Objects[i] = generateFitFunction( (TH1F*)L1XE50_Efficiency_Objects[i] );
    }
}//}}}
